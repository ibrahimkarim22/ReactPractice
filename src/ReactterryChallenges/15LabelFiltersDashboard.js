  ...
  const handleLabelClick = (animalClass) => {
    setLabelClicked((prev) => {
      if (prev.includes(animalClass)) {
        return prev.filter((c) => c !== animalClass)
      } else {
  ...
  return (
    <Wrapper>
      <div data-testid='labels-wrapper-id' className='label-container'>
        {animalClasses.map((animalClass) => (
          <div
            ...
            className={labelClicked.includes(animalClass) ? 'label-clicked' : 'label'}
            key={animalClass}
      ...
      <div data-testid='tile-container-id' className='tile-container'>
        {(labelClicked.length === 0 ? animalData : animalData.filter(animal => labelClicked.includes(animal.class)))
          .map((animal) => (
